...
function parserGenerator() {
        const tokensProc = (x:ParsedTree[])=>{
            ...
            return r;
                ...
                const paren = Rec(()=>node("paren",choice(
                        sequence(node("{",char("{")),tokens,node("}",char("}"))),
                        sequence(node("(",char("(")),tokens,node(")",char(")"))),
                        sequence(node("[",char("[")),tokens,node("]",char("]"))),
                    )));
            const token:Parser<ParsedTree> = choice(sep,symbol,literal,identifier,paren);
        const tokens:Parser<ParsedTree> = node("tokens",filter(proc(many(token),tokensProc),""));
