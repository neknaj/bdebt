import {InputState,ParseResult,MemoCache,Parser,Input,sequence,choice,many,many1,sepBy,optional,eof,char,string,reg,proc,join,ParsedTree,node,filter,memoize,Rec} from "./parserCombinator.js";
...
function parserGenerator() {
    ...
    const sep = choice(char(" "),char("\n"));
        ...
        const object = node("object",choice(
                Rec(()=>filter(sequence(node("",char("[")),node("",many(sep)),node("object_elm",sepBy(filter(sequence(choice(identifier,$string),node("",many(sep)),node("",char(":")),node("",many(sep)),expr)),node("",many(sep)),"allow")),node("",char("]"))))),
                Rec(()=>filter(sequence(node("",char("[")),node("",many(sep)),node("",char(":")),node("",many(sep)),node("",char("]"))))),
            ));
    ...
    const literal = node("literal",choice(bool,number,$string,array,object));
        ...
        const variable = node("variable",identifier);
            const funcCallRPN = Rec(()=>node("funcCallRPN",filter(choice(
                    filter(sequence( node("",string("(")) , node("",many(sep)) , expr_ , node("",many(sep)) , node("",char(":")) , node("",many(sep)) , expr , node("",many(sep)) , node("",char(")")) )),
                ))));
            const funcCallPN = Rec(()=>node("funcCallPN",filter(choice(
                    filter(sequence( node("",string("(")) , node("",many(sep)) , node("",string(":")) , node("",many(sep)) , expr , node("",many1(sep)) , expr_ , node("",many(sep)) , node("",char(")")) )),
                    filter(sequence( node("",string("(")) , node("",many(sep)) , expr , node("",many(sep)) , node("",char(":")) , node("",many(sep)) , expr_ , node("",many(sep)) , node("",char(")")) )),
                ))));
